...
fvec_t* allocateFVec(fvec_t* vec, int len){
    ...
    vec->len  = len; 
    vec->vals = (myfloat*) calloc(len, sizeof(myfloat));
    return vec;
}
...
fvec_t* reallocateFVec(fvec_t* vec, int new_len){
    ...
    vec->len  = new_len;
    vec->vals = (myfloat*) realloc(vec->vals, sizeof(myfloat)*new_len);
    return vec;
}
...
ivec_t* allocateIVec(ivec_t* vec, int len){
    if (vec == NULL)
        vec = (ivec_t*) malloc(sizeof(ivec_t));
    vec->len  = len; 
    vec->vals = (int*) calloc(len, sizeof(int));
    return vec;
}
...
ivec_t* reallocateIVec(ivec_t* vec, int new_len){
    ...
    vec->len  = new_len;
    vec->vals = (int*) realloc(vec->vals, sizeof(int)*new_len);
    return vec;
}
...
fmatCSR_t* allocateFMatCSR(fmatCSR_t* mat, int nb_non_zero, int nb_rows, int nb_cols){
    if( mat == NULL){
        mat = (fmatCSR_t*) malloc(sizeof(fmatCSR_t));
    }  
    mat->nb_cols     = nb_cols;
    mat->nb_rows     = nb_rows;
    mat->nb_non_zero = nb_non_zero;

    mat->vals  = (myfloat*) calloc(nb_non_zero       , sizeof(myfloat));
    mat->col_i = (int*)     calloc(nb_non_zero       , sizeof(int)    );
    mat->row_i = (int*)     calloc((mat->nb_rows + 1), sizeof(int)    );
    return mat;
}
...
fmatCOO_t* allocateFMatCOO(fmatCOO_t* mat, int nb_non_zero, int nb_rows, int nb_cols){
    if( mat == NULL){
        mat = (fmatCOO_t*) malloc(sizeof(fmatCOO_t));
    }  
    mat->nb_cols = nb_cols;
    mat->nb_rows = nb_rows;
    mat->nb_non_zero = nb_non_zero;
    ...
    mat->vals  = (myfloat*)calloc(nb_non_zero, sizeof(myfloat));
    mat->col_i = (int*)    calloc(nb_non_zero, sizeof(int)  );
    mat->row_i = (int*)    calloc(nb_non_zero, sizeof(int)  );
    return mat;
}

fmatD_t* allocateFMatD(fmatD_t* mat, int nb_rows, int nb_cols){
    if( mat == NULL){
        mat = (fmatD_t*) malloc(sizeof(fmatD_t));
    }  
    mat->nb_cols = nb_cols;
    mat->nb_rows = nb_rows;

    mat->vals  = (myfloat*) calloc(nb_cols*nb_rows, sizeof(myfloat));
    return mat;
}
...
fmatCSR_t* initializeRandomFMatCSR(int nb_rows, int nb_cols, myfloat density, int rand_seed, myfloat rand_min, myfloat rand_max){
    ...
    char* used_indices = (char*) calloc(total_elements,  sizeof(char)); // bitmap goes fast like brrrrr
...
fmatD_t* initaliseRandomFMatD(fmatD_t* mat, int rand_seed, myfloat rand_min, myfloat rand_max){
    ...
    assert(mat->vals);
    srand(rand_seed);
    for(int i = 0; i < mat->nb_cols*mat->nb_rows; i++){
        mat->vals[i]  = getRandomFloat(rand_min, rand_max);
    }
    return mat;
}
...
fmatCSR_t* initializeFMatCSRFromDenseMat(fmatD_t* D, fmatCSR_t* CSR, myfloat tolerance){
    ...
    if( CSR == NULL){
        CSR = (fmatCSR_t*) malloc(sizeof(fmatCSR_t));
    }
...
fvec_t* matVecProdCSRf(fmatCSR_t* mat, fvec_t* vec, fvec_t* sol_vec){
    ...
    if (sol_vec->vals == NULL){
        ...
        sol_vec->len = mat->nb_cols;
        sol_vec->vals = (myfloat*) calloc( mat->nb_cols, sizeof(myfloat));
    }
...
fvec_t* matTransposeVecProdCSRf(fmatCSR_t* mat, fvec_t* vec, fvec_t* sol_vec){
    ...
    if (sol_vec->vals == NULL){
        printf("Allocating sol vec\n");
        sol_vec->len = mat->nb_rows;
        sol_vec->vals = (myfloat*) calloc( mat->nb_rows, sizeof(myfloat));
    }
